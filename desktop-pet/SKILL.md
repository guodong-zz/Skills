...
Treat web-generated images, screenshots, checkerboard-background images, partial grids, single references, and existing pet folders as source material that must be inspected. Do not assume the input is already a valid spritesheet. In particular, a visually transparent checkerboard is not proof of transparency: browser-generated PNG files may contain an alpha channel whose values are all 255.
Workflow:
```text
Inspect
...
Use local image processing for alpha repair, background removal, cropping, resizing, mirroring, slicing, reordering, and format conversion. These operations must not trigger image generation.
...
Treat fake transparency as a repairable preprocessing defect, not as a reason to regenerate an otherwise valid character or action strip. If the action content is good, preserve it and repair the background locally.
...
5.1 Deterministic Transparency Repair Protocol
Transparency repair is a local preprocessing task. Do not use image generation merely to obtain a transparent background. Prefer Pillow, ImageMagick, OpenCV, or another deterministic local image-processing tool.
Run an alpha audit before repair:
```text
record image mode
record alpha min/max
count pixels with alpha = 0
count pixels with 0 < alpha < 255
count pixels with alpha = 255
inspect border and corner pixels
inspect whether the apparent checkerboard is encoded in RGB
```
Interpretation:
```text
RGB image -> no alpha, background must be removed
RGBA with alpha min = max = 255 -> fully opaque; apparent transparency is fake
RGBA with some alpha < 255 -> real alpha exists, but contamination may still remain
```
Repair priority:
```text
1. Preserve the original sprite pixels whenever possible.
2. Remove only background that is connected to the outer canvas border when feasible.
3. For checkerboards, estimate repeating background colors/pattern from corners and border regions; do not globally delete all white or gray pixels.
4. For flat or near-flat backgrounds, use border-connected color-distance removal rather than global thresholding.
5. Protect light clothing, skin highlights, fan paper, white hair, and anti-aliased edges from accidental deletion.
6. Refine the alpha edge only around the foreground boundary; avoid broad blur that changes the sprite silhouette.
7. Set RGB values of fully transparent pixels to neutral/zero values to prevent color fringe during WebP rendering.
8. Save an intermediate RGBA PNG before final WebP conversion.
```
Do not use a naive rule such as `all near-white pixels -> alpha 0` when the character contains white or pale clothing.
After repair, verify:
```text
alpha is not uniformly 255
outer empty canvas contains alpha = 0
character pixels remain opaque or softly anti-aliased as appropriate
no checkerboard pattern remains in visible RGB regions
no white/gray halo around hair, dress, hands, fan, or feet
no holes were cut into pale character regions
transparent RGB residue is cleaned
```
If deterministic removal damages the character, revert to the original source and try a safer local method. Escalate to segmentation only when simpler border-connected methods fail. Image regeneration remains the last resort.
5.2 Reuse By Mirroring When Semantically Safe
A validated directional movement strip may be mirrored locally to create the opposite direction when all of the following are true:
```text
movement cycle is otherwise valid
character design is visually symmetric enough for mirroring
no readable text is present
no one-sided accessory, scar, hair ornament, weapon, bag, or costume detail would become semantically wrong
no runtime convention requires distinct hand dominance or asymmetric orientation
```
Example:
```text
valid Row 1 right-walk strip
-> horizontal mirror each frame
-> candidate Row 2 left-walk strip
-> semantic QA
```
If mirroring would alter identity-defining asymmetric details, generate or repair Row 2 separately. Mirroring is preferred over image generation only when it preserves character semantics.
...
alpha channel is not uniformly opaque when empty canvas exists
empty canvas regions are truly transparent, not rendered checkerboard pixels
...
[PASS] Transparency and background are valid; alpha audit confirms true transparency rather than an opaque checkerboard
